...
/** A rewrite rule that searches for a left-hand side using a [[Searcher]], and applies a right-hand
  * side using an [[Applier]].
  */
class Rewrite(val name: String, val searcher: Searcher, val applier: Applier) {
  ...
  def apply(egraph: EGraph, shc: Substs)(matches: Vec[SearchMatches[shc.Subst]]): Vec[EClassId] =
    applier.applyMatches(egraph, shc)(matches)
}

/** The left-hand side of a [[Rewrite]] rule. A searcher is something that can search the [[EGraph]]
...
trait Searcher {
  ...
  def searchEClass(egraph: EGraph, shc: Substs, eclass: EClassId): Option[SearchMatches[shc.Subst]]
...
/** The right-hand side of a [[Rewrite]] rule. An applier is anything that can use a [[Subst]] to
  * modify the [[EGraph]].
  */
trait Applier {
  ...
  def applyOne(egraph: EGraph, eclass: EClassId, shc: Substs)(subst: shc.Subst): Vec[EClassId]

  // the substitutions inside `matches` may be modified
  def applyMatches(egraph: EGraph, shc: Substs)(
...
/** The result of searching over one [[EClass]]. Note that multiple substitutions can have been
  * found.
  */
...
abstract class ConditionalApplier(
  ...
  override def applyOne(egraph: EGraph, eclass: EClassId, shc: Substs)(
      subst: shc.Subst
  ): Vec[EClassId] = {
    if (cond(egraph, eclass, shc)(subst)) { applier.applyOne(egraph, eclass, shc)(subst) }
    else { Vec() }
  }
}
...
/** An [[Applier]] that shifts the DeBruijn indices of a variable.
  * @note
  *   It works by extracting an expression from the [[EGraph]] in order to shift it.
  */
...
case class ShiftedDataTypeApplier(
    v: DataTypePatternVar,
    newV: DataTypePatternVar,
    shift: Type.Shift,
    cutoff: Type.Shift,
    applier: Applier
) extends Applier {
...
/** An [[Applier]] that performs beta-reduction.
  * @note
  *   It works by extracting an expression from the [[EGraph]] in order to beta-reduce it.
  */
case class BetaExtractApplier(body: PatternVar, subs: PatternVar) extends Applier {
  override def patternVars(): Set[Any] =
    Set(body, subs)
...
/** An [[Applier]] that performs beta-reduction.
  * @note
  *   It works by directly transforming [[EClass]]es.
  */
case class BetaApplier(body: PatternVar, subs: PatternVar) extends Applier {
  override def patternVars(): Set[Any] =
    Set(body, subs)
...
/** An [[Applier]] that performs beta-reduction for nat-dependent functions.
  ...
  *   It works by directly transforming [[EClass]]es.
  */
case class BetaNatApplier(body: PatternVar, subs: NatPatternVar) extends Applier {
  override def patternVars(): Set[Any] =
    Set(body, subs)
...
/** An [[Applier]] that performs beta-reduction for nat-dependent functions.
  ...
  *   It works by extracting an expression from the [[EGraph]] in order to beta-reduce it.
  */
case class BetaNatExtractApplier(body: PatternVar, subs: NatPatternVar) extends Applier {
  override def patternVars(): Set[Any] =
    Set(body, subs)
...
case class ComputeNatCheckApplier(
  ...
  override def cond(egraph: EGraph, id: EClassId, substs: Substs)(subst: substs.Subst): Boolean = {
    ...
    ComputeNat.toNamed(egraph, v, substs)(subst) == ComputeNat.toNamed(egraph, expected, substs)(
      subst
    )
  }
}
...
case class ComputeNatApplier(
  ...
  override def applyOne(egraph: EGraph, eclass: EClassId, shc: Substs)(
    ...
    val actualValue = Nat.fromNamed(ComputeNat.toNamed(egraph, value, shc)(subst), computeScope)
...
private object ComputeNat {
  ...
  def toNamed(egraph: EGraph, n: NatPattern, shc: Substs)(subst: shc.Subst): rct.Nat = {
    n match {
...
/** An [[Applier]] that vectorizes a scalar function.
  * @note
  *   It works by extracting an expression from the [[EGraph]] in order to vectorize it.
  */
case class VectorizeScalarFunExtractApplier(
  ...
  private def vectorizeExpr(
      expr: ExprWithHashCons,
      n: NatId,
      eg: EGraph,
      vEnv: Set[Int]
  ): Option[ExprWithHashCons] = {
    import rise.core.{primitives => rcp}
    expr.node match {
      case Var(i) if vEnv(i) =>
        for { tv <- vecDT(expr.t, n, eg) } yield ExprWithHashCons(Var(i), tv)
      case Var(_) => None
      case App(f, e) =>
        for {
          fv <- vectorizeExpr(f, n, eg, vEnv); ev <- vectorizeExpr(e, n, eg, vEnv)
        } yield ExprWithHashCons(App(fv, ev), eg(fv.t).asInstanceOf[FunType[TypeId]].outT)
      case Lambda(e) =>
        for {
          ev <- vectorizeExpr(e, n, eg, vEnv.map(_ + 1) + 0);
      ...
      case Literal(_) | NatLiteral(_) | IndexLiteral(_, _) =>
        for { tv <- vecDT(expr.t, n, eg) } yield ExprWithHashCons(
          App(
            ExprWithHashCons(
              Primitive(rcp.vectorFromScalar.primitive),
              eg.add(FunType(expr.t, tv))
            ),
            expr
          ),
          tv
      ...
      case Primitive(rcp.add() | rcp.mul() | rcp.fst() | rcp.snd()) =>
        for { tv <- vecT(expr.t, n, eg) } yield ExprWithHashCons(expr.node, tv)
      case Primitive(_) => None
      case Composition(f, g) =>
        for {
          fv <- vectorizeExpr(f, n, eg, vEnv); gv <- vectorizeExpr(g, n, eg, vEnv)
        } yield ExprWithHashCons(
          Composition(fv, gv),
          eg.add(
            FunType(
  ...
  private def vecT(t: TypeId, n: NatId, eg: EGraph): Option[TypeId] = {
    t match {
      ...
      case ndt @ NotDataTypeId(_) =>
        eg(ndt) match {
          case FunType(inT, outT) =>
            for { inTV <- vecT(inT, n, eg); outTV <- vecT(outT, n, eg) } yield eg.add(
              FunType(inTV, outTV)
            )
          case NatFunType(t)         => ???
          case DataFunType(t)        => ???
  ...
  private def vecDT(t: DataTypeId, n: NatId, eg: EGraph): Option[DataTypeId] = {
    eg(t) match {
      ...
      case PairType(a, b) =>
        for { a2 <- vecDT(a, n, eg); b2 <- vecDT(b, n, eg) } yield eg.add(PairType(a2, b2))
