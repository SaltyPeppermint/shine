...
case class DataTypePatternVar(index: Int) extends DataTypePattern {
  override def toString: String = s"?d$index"
}
...
object DataType {
  def fromNamed(dt: rct.DataType, scope: Expr.Scope = Expr.Bound.empty): DataType = {
    DataType(dt match {
      ...
      case _: rcdt.DepArrayType | _: rcdt.DepPairType[_, _] | _: rcdt.NatToDataApply |
          _: rcdt.FragmentType | _: rcdt.ManagedBufferType | _: rcdt.OpaqueType =>
...
sealed trait TypeNode[+T, +N, +DT] {
  def map[TO, NO, DTO](ft: T => TO, fn: N => NO, fdt: DT => DTO): TypeNode[TO, NO, DTO] =
    this match {
...
sealed trait DataTypeNode[+N, +DT] extends TypeNode[Nothing, N, DT] {
  def map[NO, DTO](fn: N => NO, fdt: DT => DTO): DataTypeNode[NO, DTO] =
    this match {
